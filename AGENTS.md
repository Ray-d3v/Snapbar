...
- The idle affordance must remain approximately 80–96 px wide and 28–30 px high. Expand into the bounded 288 × 50 px island with a visible concave shoulder/root at the caption bottom; the 20 px drop is part of that island, not a separate menu. Keep all controls on one horizontal row.
- SaveInfo currently uses the normal 288 × 50 logical-pixel envelope, 288 × 49 visible surface, and 304 × 52 HWND/input geometry; inline retains its 29-pixel height, while presenter attachment retains its existing 49-pixel expanded height. Prefer concise content and layout adjustments within this baseline. If adequate padding cannot fit, modestly increase both width and height by approximately the same proportion; never stretch just one axis substantially. Any size change must keep painting, HWND bounds, and native input geometry synchronized, preserve the title-bar attachment, and avoid Teams controls and content. The current page transition uses only a short 140 ms fade.
- Treat overlay dimensions as logical pixels. Scale the HWND, painted surface, native region, corners, and hit targets with the target monitor's Windows DPI; do not scale them from raw monitor resolution or physical panel size.
...
- Keep idle, compact, and expanded caption-contained areas visually transparent like native caption cells. Paint only the expanded drop with the sampled Teams title-bar color, preserving the square top, concave shoulders, and rounded bottom. Do not force the expanded material to black; derive text and control contrast from the same sampled color.
