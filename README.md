...
タイトルバーの表示パターンは起動引数で選べます。引数なしでは、ホバー時に操作列がタイトルバーの下側へ少し広がります。`--inline-titlebar`（短縮形`--inline`）を付けると、ホバー時に操作列がタイトルバー内だけで横方向へ展開します。

GitHub Releasesのインストーラー版は、Windowsサインイン時の自動起動、インストール完了時の起動、スタートメニュー、任意で作成するデスクトップショートカットのすべてを引数なしで起動します。そのため、通常のインストール操作では、ホバーすると操作列がタイトルバー下へ約16px広がる表示が既定になります。タイトルバー内だけで完結する表示へ切り替えたい場合は、インストール先の`Snapbar.exe`へ`--inline-titlebar`を付けて起動します。
...
約16ms（60Hz表示の約1フレーム）ホバーすると、タイトルバーへ接続したまま中央を基準に、固定`272 × 46px`エンベロープ内で操作列を展開します。上端1 logical pxは終始透明に保ち、可視面を最大`272 × 45px`（タイトルバー内29px＋下方向16px）へ制限するため、Teamsの上部枠線は待機時・展開中・縮小中も消えません。展開面は初動約10%でタイトルバーの実表示色まで立ち上がり、ライトテーマでは暗い文字と操作面、ダークテーマでは明るい文字と操作面へ自動反転します。タイトルバー下端から根本が四分円状に内側へカーブし、下角を約`R8px`で丸めた同色の面が約`16px`下へ膨らみます。形状と内容は約150〜200msで展開し、ピークでわずかに膨らんでから約300msまでに収束します。下へ別メニューは開かず、中央の安全領域からも動きません。

未ホバー時と展開後で撮影ボタンの見た目とX座標を固定しています。ホバー後もカーソルを横へ追い直す必要はありません。

展開後に各ボタンへカーソルを重ねると、左側の固定表示欄へ`撮影`、`PNG保存 ON/OFF`、`再検出`、`終了`のように機能名を表示します。別のツールチップウィンドウは開かないため、Teamsとの前後関係やホバー判定は変わりません。
...
展開後の各操作は`30 × 30 logical px`のクリック領域を維持しながら、見た目のボタン面を`28 × 28 logical px`へ一段内側に収めています。これにより隣接ボタン間には見た目上約8pxの間隔ができ、既定表示では操作列を1px上へ寄せて曲面側の下余白も確保します。
...
- 撮影成功後、実際に取得した共有領域へ短い白いフラッシュを表示します。通常のDebug／Release起動では、このフラッシュも画面共有・画面録画へ映ります。
...
- タイトルバーUIのTeams追従は位置が変化した場合だけ`SetWindowPos`を実行します。アニメーションのフレーム通知では形状と入力領域だけを更新し、Teamsのウィンドウ情報取得、配置、Zオーダー確認は既存周期の絶対期限で実行するため、連続フレームによって追従確認が後ろ倒しになりません。
