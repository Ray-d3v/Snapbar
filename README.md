...
GitHub Releasesのインストーラー版は、Windowsサインイン時の自動起動、インストール完了時の起動、スタートメニュー、任意で作成するデスクトップショートカットのすべてを引数なしで起動します。そのため、通常のインストール操作では、ホバーするとタイトルバー下へ約16pxせり出すDynamic Island風の表示が既定になります。タイトルバー内だけで完結する表示へ切り替えたい場合は、インストール先の`Snapbar.exe`へ`--inline-titlebar`を付けて起動します。
