...
- 左側には接続状態と撮影状態を表示します。複数のTeams候補がある場合はクリックで切り替えます。
- 中央の赤いカメラボタンで撮影します。
- 右端の`…`へマウスを乗せるかクリックすると設定メニューが開きます。
...
4. UI Automationが広い会議ステージしか返さない場合は、画像を低解像度で解析し、行・列方向の連続密度から実際の共有面を検出します。これにより、右側の参加者列と上側の参加者ストリップを共有面から分離します。
