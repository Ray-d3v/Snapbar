...
- After the configured disclosure delay, the surface expands to a 272 × 46 logical-pixel island whose material matches the representative color sampled from the visible Teams title bar. Its top remains connected to the title bar, its shoulders follow a concave quarter-circle from the caption bottom (10 logical-pixel depth with a 16 logical-pixel inset), and its bottom corners use a 6 logical-pixel radius. The island reserves a 16 logical-pixel drop below the caption.
